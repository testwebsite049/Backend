...
const { getLocalIPv4Address } = require('../Comman/common'); 

exports.register = async (req, res) => {
    const { username, email, password, ip_address } = req.body;
    try {
        const serverIP = getLocalIPv4Address();
        // Hash the password
...
exports.loginUser = async (req, res) => {
    try {
        ...
        res.status(200).json({ message:'Login Successfully', token, serverIP });
    } catch (err) {
        res.status(400).json({ message: err.message });
    }
};
...
exports.searchGoogleMaps = async () => {
    try {
